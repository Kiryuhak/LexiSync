...
export function matchesSite(hostname: string, pattern: string): boolean {
    ...
    return Boolean(normalizedHost && normalizedPattern
...
export function resolveStyleProfile(
    ...
    if (hostname) {
        const automatic = profiles.find((profile) => normalizeSitePatterns(profile.sites).some((site) => matchesSite(hostname, site)));
