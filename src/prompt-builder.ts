...
export function buildPromptPayload(msg: PromptRequest, settings: PromptSettings): PromptPayload {
    ...
    if (msg.mode === 'spellcheck') {
        systemPrompt +=
            ' Ты действуешь как высокоточный консервативный корректор текста. ' +
            'Исправь исключительно реальные орфографические, грамматические, пунктуационные ошибки, опечатки, падежи, дефисы, частицы «не/ни» и согласование слов. ' +
            'Внимательно проверь нормативное написание каждого слова и исправь все найденные ошибки, а не только первую (например: «сиводня харошая пагода» → «сегодня хорошая погода»). ' +
            'СТРОГИЕ ПРАВИЛА: ' +
            '1. Не перефразируй предложения и не заменяй грамотные слова на синонимы. ' +
            '2. Сохраняй исходный порядок слов, стиль, смысл, структуру и факты. ' +
            '3. Сохраняй язык оригинала (ни в коем случае не переводи). ' +
