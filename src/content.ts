...
function showToolbarMenu(x: number, y: number): void {
    ...
    moreDropdown.appendChild(
        createDropdownItem(ICONS.translate, t('translate', 'Перевести'), () => handleActionClick('translate')),
    );
    moreDropdown.appendChild(
        createDropdownItem(ICONS.keyboard, t('fixLayout', 'Исправить раскладку'), () => handleActionClick('layout')),
    );
    moreDropdown.appendChild(
...
function showAIMenu(x: number, y: number): void {
    ...
    popupUI.appendChild(
        createMenuBtn(ICONS.spell, t('fixErrors', 'Исправить ошибки'), () => handleActionClick('spellcheck'), 'Alt+R'),
    );
    popupUI.appendChild(
        createMenuBtn(ICONS.style, t('rewriteText', 'Переписать текст'), () => handleActionClick('style'), 'Alt+Y'),
    );
    popupUI.appendChild(
        createMenuBtn(ICONS.emoji, t('addEmoji', 'Подобрать эмодзи'), () => handleActionClick('emoji'), 'Alt+T'),
    );

    void chrome.storage.local.get({ customCommands: [] }).then((stored) => {
        if (popupUI !== menuPopup || !Array.isArray(stored.customCommands) || stored.customCommands.length === 0)
            return;
