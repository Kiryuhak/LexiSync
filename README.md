...
- **📊 Поддержка Google Таблиц и редакторов:** Автопроверка и подсветка опечаток в ячейках Google Таблиц (`.cell-input`), строке формул, Google Документах, Notion, Slack и Telegram Web.
- **🔴 Мгновенная нативная подсветка:** Автоматическая активация браузерной проверки `spellcheck="true"` прямо во время набора текста в ячейках таблиц.
- **⚡ Горячая клавиша `Ctrl+Enter` / `Cmd+Enter`:** Мгновенное применение исправления из карточки подсказки без потери фокуса ячейки.
- **🪟 Адаптивная вёрстка кнопок:** Исправлен перенос кнопок в компактном окне результата, кнопка «Отменить замену» теперь идеально вписывается в границы карточки на любых экранах.
