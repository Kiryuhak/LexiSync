...
- **🧭 Стабильное положение окон:** Меню и окно результата больше не смещаются в угол в Telegram и на сложных страницах с вложенными или масштабированными модальными областями.
- **📐 Более аккуратный компактный интерфейс:** Длинные названия AI-команд переносятся внутри меню, а карточка «Этот сайт» полностью выровнена с основными действиями popup.
- **🌍 Понятный контроль сайтов:** Даже при включённом доступе ко всем страницам LexiSync можно отдельно отключить на текущем сайте, не затрагивая остальные домены.
- **🔒 Усиленная приватность:** До загрузки настроек расширение не запускает обработку; отключённые сайты не выполняют автопроверку и не могут случайно отправить текст в API.
- **⚡ Меньше нагрузки на браузер:** Service worker хранит в памяти только необходимые рабочие параметры, а статистика истории считается в IndexedDB без чтения самих текстов.
- **🎨 Безопасные темы и перенос настроек:** Оформление больше не влияет на CSS веб-страниц, а экспорт и импорт сохраняют все современные стили и параметры персонализации.
- **📦 Надёжный выпуск:** Архивы Chrome и Firefox создаются один раз, проходят одинаковый набор тестов и только после этого отправляются в магазины.

## ✨ Что нового в 5.3.0

- **🎨 Новые визуальные стили оформления:** Добавлены ультрасовременные стили «Vision Aurora» (пространственное стекло с радужным преломлением и неоновыми акцентами) и «Silk Obsidian» (100% непрозрачный бархатный монолит в стиле Linear / Apple AI, гарантирующий нулевое сливание с любыми фонами).
- **💬 Умный ответ (Smart Reply):** Быстрое составление конструктивных ответов на сообщения с интерактивными чипами («Согласиться», «Вежливо отказать», «Уточнить детали», «Предложить альтернативу»).
- **💡 Объясни просто (Explain Simply):** Разъяснение сложных концепций, юридических терминов и аббревиатур понятным языком с наглядными жизненными примерами.
- **📄 Очистить и форматировать (Clean & Format):** Мгновенная очистка текста от разорванных переносов строк при копировании из PDF, удаление дублирующихся пробелов и структурирование.
- **📖 Персональный словарь в 1 клик:** Добавление исключений и редких слов в пользовательский словарь прямо из карточек автопроверки.
- **⚡ Офлайн-правила типографики:** Мгновенное локальное форматирование кавычек-ёлочек («»), длинных тире (—), неразрывных пробелов и дробей без запросов к серверу.
- **🔍 Улучшенный Live-Proofread:** Интеллектуальная защита границ слов в диффе (устранено дробление букв внутри слов) и выравнивание элементов интерфейса.
...
- **🎨 Варианты интерфейса:** Liquid Glass, MagicOS, Aurora Glass, Material Design 3, Flutter Clean, Vision Aurora и Silk Obsidian со светлой, тёмной и системной темами.
