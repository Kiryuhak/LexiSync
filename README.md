...
<p align="center">
  ...
  ·
  <a href="https://github.com/Kiryuhak/LexiSync/releases/tag/v5.6.6"><strong>Релиз v5.6.6</strong></a>
  ·
  <a href="docs/RELEASING.md">Инструкция по выпуску</a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/версия-5.6.6-blue.svg?style=flat-square" alt="Версия 5.6.6">
...
<h2 align="center">✨ Интерфейс LexiSync 5.6.6</h2>
...
- **Исправление отображения кнопки «Заменить»:** устранён конфликт специфичности CSS светлой темы в компактном режиме, приводивший к скрытию белого текста кнопки на светло-сером фоне.
- **Гарантированная видимость и контрастность действий:** кнопка получила явный лейбл `.lexisync-btn-label`, высокую контрастность (`#ffffff` на `#10b981`), адаптивность под тёмную тему и корректные состояния hover/disabled.
- **Единая модель действий результата:** согласованное первичное действие замены во всех режимах проверки с кнопками «Заменить», «Вставить ниже», «Отклонить» и поддержкой отмены (Undo).
- **Локализация:** добавлен отдельный ключ `replace` («Заменить» / «Replace») в словари русского и английского языков.
- **Автоматические регрессионные тесты:** тесты Unit 16b, Test 69 и Test 70 гарантируют наличие видимого текста или accessible-имени для всех кнопок действий и подтверждают корректную замену в DOM.
- **Двуязычная поддержка в CI:** расширена устойчивость автоматических тестов к запуску в различных локалях окружения.
