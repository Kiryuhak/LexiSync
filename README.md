...
- **⚡ Текстовые макросы:** создавайте короткие команды вроде `/mail` и разворачивайте их прямо в обычных полях ввода. Пароли, платёжные и другие чувствительные поля автоматически исключены.
- **🧰 Больше инструментов без лишних запросов:** смена регистра и очистка текста работают локально, а новые команды помогают продолжить мысль, превратить заметки в документ и подобрать заголовки.
- **🕒 Улучшенная история:** до 500 локальных записей, безопасный импорт без дублей и экспорт в JSON, CSV или Markdown.
- **📊 Понятная статистика:** в настройках появились процент успешных запросов и самая используемая команда; повреждённые локальные счётчики больше не искажают показатели.
- **🎨 Стабильный UI и UX:** компактные окна корректно остаются в пределах экрана после изменения размера, настройки блокируются только на время восстановления, а сочетания клавиш открываются в правильном разделе Chrome или Firefox.
- **🔒 Меньше разрешений и рисков:** удалён неподдерживаемый эксперимент голосового ввода; расширение по-прежнему не запрашивает доступ к микрофону и не добавляет внешнюю телеметрию.
