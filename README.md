...
- Настройки стали красочнее и понятнее: добавлены спокойная анимация, цветные вкладки и короткие пояснения функций.
- Номер версии открывает компактную историю всех обновлений с поиском и отметкой текущего выпуска.
- Повтор запроса, быстрые уточнения и персональные подсказки стали стабильнее.
- Автопроверка сразу останавливается после отключения функции или добавления сайта в исключения.
- Firefox XPI создаётся только после полной проверки production-сборок и совпадения контрольной суммы.
...
- **Устойчивость API:** Корректная обработка ограничений частоты запросов (ошибка 429), мягкое восстановление и таймер обратного отсчёта для пользователя.
