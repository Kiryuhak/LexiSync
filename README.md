...
- **Стабильный spellcheck через Cloudflare:** для короткого исправления текста отключены скрытые рассуждения `GLM-4.7-Flash`, которые могли занять весь лимит ответа и оставить результат пустым.
- **Надёжный fallback после Mistral 429:** запрос однократно переключается на Cloudflare, а следующие команды во время cooldown не тратят время на заведомо недоступный Mistral.
- **Экономная проверка статуса:** состояние AI-сервисов проверяется только по кнопке и сохраняется в кэше; открытие и обычное сохранение настроек не запускают дополнительную генерацию.
...
> Текущая ветка разработки после 5.6.4 содержит локальный корректор и дополнительные проверки выделения. Эти
> изменения ещё не опубликованы в магазинах и не меняют версию установленного релиза.
