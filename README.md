...
- **✉️ Изоляция событий в модальных окнах:** Устранено сворачивание диалоговых окон написания писем (Яндекс.Почта, Gmail, Outlook) при кликах по кнопкам тулбара и меню за счет монтирования контейнера расширения в активный модальный контекст и изоляции событий `pointerdown`/`mousedown`.
...
- **📜 Адаптивный скролл меню:** Добавлено ограничение `max-height` с внутренним скроллом для гарантированного удобства на экранах любого разрешения и масштаба.
- **🎨 Повышенная плотность фонов:** Фоны всех выпадающих меню и карточек сделаны более контрастными и плотными (94–98%) во всех стилях оформления (Liquid Glass, MagicOS 11, Vision Aurora, Silk Obsidian).
- **📝 Чистый вывод выжимки:** Убраны технические префиксы `**TL;DR:**` из вывода и заголовка режима.
