...
<p align="center">
  ...
  ·
  <a href="https://github.com/Kiryuhak/LexiSync/releases/tag/v5.6.9"><strong>Релиз v5.6.9</strong></a>
  ·
  <a href="docs/RELEASING.md">Инструкция по выпуску</a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/версия-5.6.9-blue.svg?style=flat-square" alt="Версия 5.6.9">
...
<h2 align="center">✨ Интерфейс LexiSync 5.6.9</h2>
...
- **Сохранение результата Спеллера при сбое AI:** ошибка, лимит 429 или некорректный ответ AI не удаляют орфографический результат, а кнопка «Заменить» остаётся активной.
...
- **Улучшенная валидация качества (Sanity Check):** типографические диапазоны `5-10` / `5–10` / `5—10`, разрядные пробелы и десятичные разделители распознаются без ложных отказов, а реальные изменения цифр, дат, версий, IP и телефонов блокируются.
- **Безопасная позиционная замена:** исправления применяются по offsets API; неоднозначные варианты не подставляются автоматически.
...
- **🔤 Орфография через Яндекс.Спеллер:** официальный облачный API исправляет русские и смешанные русско-английские опечатки. [Проверка правописания: Яндекс.Спеллер](https://api.yandex.ru/speller/).
- **✨ Опциональная проверка через AI («Проверить через AI»):** результат Спеллера можно передать в Mistral / Cloudflare для проверки грамматики, пунктуации, согласования и контекста.
- **🔀 Три режима проверки:** «Орфография» (Яндекс.Спеллер), AI (Mistral → Cloudflare при разрешённом fallback) и гибридный (Спеллер → AI для сложных случаев).
...
- **API:** WebExtensions Manifest V3, официальный JSON API Яндекс.Спеллера, Mistral AI REST API, Cloudflare Workers AI REST API.
...
2. **Настройте провайдеров:**
    ...
    - В поле **Проверка текста** выберите _Гибридная_ (по умолчанию), _Только Спеллер_ или _Только AI_. Для Спеллера ключ не требуется, но текст отправляется Яндексу через интернет.
...
- **Гибридная проверка текста:** после явной команды Яндекс.Спеллер первым проверяет орфографию, затем исправленный им текст последовательно передаётся выбранному AI для грамматики, пунктуации и контекста. Провайдеры не вызываются параллельно.
- **Только Спеллер:** отправляет текст в Яндекс.Спеллер, исправляет однозначные опечатки, а неоднозначные варианты оставляет для ручной проверки. Контекстную грамматику и стилистику этот режим не обещает.
...
- Все обращения к Яндекс.Спеллеру и AI выполняются фоновым процессом расширения напрямую по HTTPS (`https://speller.yandex.net/`, `https://api.mistral.ai/`, `https://api.cloudflare.com/`). В проекте отсутствуют промежуточные серверы разработчика и внешняя телеметрия.
- Личный словарь хранится на устройстве и применяется как список исключений. Он не включается в запросы Яндекс.Спеллера, Mistral или Cloudflare.
...
Орфографическая проверка предоставляется внешним сервисом на условиях [API Яндекс.Спеллера](https://yandex.ru/legal/speller_api/ru).
