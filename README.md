...
- **🛡️ Рабочая защита персональных данных:** Переключатель маскировки теперь надёжно сохраняется и синхронизируется. Email, телефоны, карты, IP-адреса и секретные ключи скрываются до запроса, а после ответа исходные значения безопасно возвращаются в готовый текст.
- **🎨 Читаемый интерфейс во всех темах:** Исправлен контраст руководства, окна сброса и библиотеки команд в светлой, тёмной, MagicOS, Aurora и остальных стилях.
- **📱 Аккуратная мобильная компоновка:** Заголовок команд, галерея и руководство больше не создают горизонтальную прокрутку на узких экранах.
- **📚 Понятные шаблоны без дублей:** В галерее собраны повседневные команды для перевода, выжимки, деловой переписки и соцсетей; повторное добавление обновляет команду вместо создания копии.
- **⌨️ Улучшенная доступность:** Руководство открывается компактно, аккордеон получил корректные связи для экранных дикторов, а новые кнопки — заметный клавиатурный фокус.
- **⚡ Техническая оптимизация:** Настройка приватности включена в быстрый кэш service worker и ключ ответа, миграция безопасно активирует защиту для существующих установок.
