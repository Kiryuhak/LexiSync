...
- **Кнопка «Проверить через AI»:** после завершения оффлайн-проверки через Local Proofreader пользователь может одним кликом отправить текст на глубокий анализ в AI-провайдер (Mistral / Cloudflare) без потери ранее найденных локальных исправлений.
- **Сохранение локального результата при сбое AI:** если облачный AI возвращает ошибку, лимит 429 или пустой ответ, локальный результат проверки не пропадает, а кнопка «Заменить» остаётся активной.
- **Персистентность кулдауна провайдеров в Manifest V3:** состояние ожидания после ошибок 429 сохраняется в `chrome.storage.session` (с fallback в `chrome.storage.local`) и не теряется при перезапуске Service Worker браузера.
- **Оптимизация Cloudflare Workers AI:** устранены периодические пустые ответы `@cf/zai-org/glm-4.7-flash` в режиме улучшения стиля за счёт отключения избыточных скрытых рассуждений (`enable_thinking: false`).
- **Улучшенная валидация качества (Sanity Check):** исключены ложные срабатывания на корректных пробелах в числах (12 500, NBSP, narrow NBSP) и переносах строк (CRLF, LF, CR) при строгой защите числовых значений от искажения.
- **Новые автоматические тесты:** добавлены E2E-тесты Test 71, Test 72 и Test 73, покрывающие интеграцию локального корректора с облачными сервисами и персистентность кулдауна.
...
- **💻 Локальная русская орфография (Local Proofreader):** полностью оффлайновый и приватный модуль на базе встроенного словаря и консервативных правил. Текст обрабатывается на устройстве за миллисекунды, гарантированно не покидает браузер и работает без сети и без API-ключей.
- **✨ Опциональная проверка через AI («Проверить через AI»):** если локальная проверка уже завершена, пользователь может одним кликом передать текст в AI (Mistral / Cloudflare) для глубокого анализа пунктуации, сложного контекста и стиля. При этом локальный результат не теряется, а статус AI-сервисов отображается прозрачно.
