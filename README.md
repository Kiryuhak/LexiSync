...
<p align="center">
  ...
  ·
  <a href="https://github.com/Kiryuhak/LexiSync/releases/tag/v5.6.5"><strong>Релиз v5.6.5</strong></a>
  ·
  <a href="docs/RELEASING.md">Инструкция по выпуску</a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/версия-5.6.5-blue.svg?style=flat-square" alt="Версия 5.6.5">
...
<h2 align="center">✨ Интерфейс LexiSync 5.6.5</h2>
...
- **Локальная проверка русской орфографии и базовой пунктуации:** проверка орфографии и типографики прямо на устройстве через встроенный компактный Bloom-фильтр (1 436 553 словоформы) без задержек и без обращения к внешним серверам.
- **Гибридный режим проверки (Hybrid Mode):** по умолчанию объединяет скорость и конфиденциальность локального анализа с интеллектуальными возможностями языковых моделей. Очевидные опечатки устраняются локально, а к Mistral или Cloudflare Workers AI запрос обращается только при необходимости.
- **Автономная работа (Offline):** полноценная работа корректора при отсутствии подключения к сети или без настроенных API-ключей. При временном сбое AI локальные исправления сохраняются и показываются пользователю с понятным уведомлением.
- **Защита снимка выделения (Selection Snapshot):** полный снимок диапазона выделения (контейнеры, смещения, исходный текст) предотвращает ошибочную замену при динамическом изменении DOM или переключении фокуса (включая Google Search, ChatGPT, сложные contenteditable и поля форм).
- **Защита от устаревших ответов (Stale Response Protection):** отмена фоновых запросов через `AbortController` и сверка идентификаторов гарантируют, что ответ на старый запрос не повредит новый выделенный фрагмент.
- **Информативное состояние «Текст уже корректен»:** устранены пустые карточки при проверке текста без изменений — пользователь видит явный статус отсутствия ошибок.
- **Надёжная валидация результатов AI:** усилен контроль целостности ответа для предотвращения случайного усечения текста, искажения ссылок, чисел, технических идентификаторов или нежелательной смены языка.
- **Headless-тестирование:** автоматические проверки Playwright Chrome и Firefox по умолчанию выполняются в headless-режиме, ускоряя цикл проверки качества.

Предыдущие изменения — в [CHANGELOG](CHANGELOG.md) и истории обновлений внутри расширения.

Методика и результаты сравнения доступных моделей Cloudflare на 28 русскоязычных примерах приведены в [отчёте benchmark](docs/ai-spellcheck-benchmark.md).
...
- **💻 Локальная русская орфография:** упакованный словарь и консервативные правила исправляют однозначные опечатки, пробелы и базовую пунктуацию на устройстве, в том числе без сети и без API-ключа.
- **🔀 Три режима проверки:** гибридный режим сначала использует локальный слой и обращается к AI только для сложных случаев; также доступны полностью локальный и AI-only варианты.
...
- **Локальный словарь:** Bloom-фильтр из русского Hunspell-словаря `dictionary-ru@3.0.0`; runtime не использует WASM или удалённый код.
...
npm run test:e2e         # Chrome E2E в headless Chromium без видимого окна
npm run test:e2e:headed  # Те же Chrome E2E с видимым браузером для отладки
npm run test:firefox     # Runtime smoke Firefox в headless-режиме
npm run test:runtime     # Headless runtime-проверки Chrome и Firefox
npm run test:runtime:headed # Явная визуальная runtime-проверка обоих браузеров
npm run test:all         # Полный автоматический quality gate без видимых браузеров
npm run test:full        # Полная проверка с видимыми браузерами для ручной оценки
npm run verify:release   # Проверка релизных архивов, манифестов и web-ext lint
```

Автоматические Playwright-проверки и создание магазинных скриншотов по умолчанию работают без видимых окон. Для визуальной отладки Chrome используйте `npm run test:e2e:headed`; переменные `PW_HEADED=1` и `PLAYWRIGHT_HEADED=1` также явно включают headed-режим. Сценарии Google Search и ChatGPT в автоматическом наборе используют локально перехваченные ответы и не открывают реальные сайты. Проверку на реальных страницах выполняйте вручную только при необходимости.
...
2. **Настройте провайдеров:**
    ...
    - В поле **Проверка текста** выберите _Гибридная_ (по умолчанию), _Только локальная_ или _Только AI_. Для локального режима ключи не требуются.
...
- **Гибридная проверка текста:** однозначные локальные исправления применяются без отправки текста провайдеру. Если остаются контекстные, грамматические или неоднозначные случаи, исправленный локальным слоем текст передаётся выбранному AI.
- **Только локальная:** работает без интернета, исправляет безопасные опечатки и типографику, а неоднозначные слова показывает для ручной проверки. Контекстную грамматику и стилистику этот режим не обещает.
- **Только AI:** сохраняет прежний маршрут Mistral/Cloudflare и требует настроенного провайдера.
...
- Локальный словарь, локальные правила и личный словарь пользователя работают на устройстве. Личный словарь не включается в запросы Mistral или Cloudflare.
...
Русский словарь `dictionary-ru` распространяется отдельно по BSD-3-Clause, а dev-инструмент `nspell` — по MIT.
Тексты лицензий и происхождение данных сохранены рядом с [исходниками словаря](vendor/dictionary-ru/README.md).
