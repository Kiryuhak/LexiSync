...
- Автопроверка и персональные подсказки не обрабатывают поля пароля, логина, почты, телефона, адреса и платёжных данных.
- После применения исправления и временных сетевых ошибок больше не возникают скрытые повторные запросы.
- Автоматическая проверка загружается только после включения, уменьшая стартовый размер content script примерно на 7,3%.
- Chrome больше не показывает ложные ошибки `cross-world extension resource mismatch` на страницах расширения.
- Результат больше не зависает в бесконечной загрузке при перезапуске фонового обработчика браузером.
- При разрыве соединения LexiSync показывает понятную ошибку и позволяет повторить запрос одним нажатием.
- Отмена запроса безопасно работает, даже если соединение уже успело закрыться.
...
- **Интерфейс:** Preact для общего каркаса окна результата и его предпросмотра, компактные DOM-модули для остальных экранов, CSS-переменные и SVG-иконки
