...
<p align="center">
  ...
  ·
  <a href="https://github.com/Kiryuhak/LexiSync/releases/tag/v5.6.3"><strong>Релиз v5.6.3</strong></a>
  ·
  <a href="docs/RELEASING.md">Инструкция по выпуску</a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/версия-5.6.3-blue.svg?style=flat-square" alt="Версия 5.6.3">
...
<h2 align="center">✨ Интерфейс LexiSync 5.6.3</h2>
...
- **Google Drive без ручных токенов:** нажмите «Синхронизировать сейчас» — Chrome или Firefox сам откроет безопасный вход Google и запросит только доступ к скрытой папке LexiSync.
- **Автоматическое восстановление доступа:** если краткоживущий токен истёк, LexiSync удалит его и один раз повторит авторизацию, не показывая технических полей пользователю.
- **Простой мастер первого запуска:** облачное восстановление теперь требует только входа Google и мастер-пароля от зашифрованной копии; локальный файл `.lexibak` остаётся запасным вариантом.
- **Аккуратная типографика настроек:** кнопка интерактивной проверки больше не соприкасается с блоком получения API-ключей.
- **Обновлённые материалы:** пересобраны русскоязычные скриншоты Chrome и Firefox и анимированная галерея README.
- **Быстрый fallback без повторного ожидания:** LexiSync запоминает 429, тайм-ауты и временные сбои отдельно для каждого провайдера, соблюдает `Retry-After` и сразу выбирает доступный резерв.
- **Не более двух AI-вызовов:** одна команда создаёт максимум один основной и один резервный запрос — скрытых серий повторов больше нет.
- **Корректные потоки и диагностика:** обновлён SSE-парсер, сообщения всегда называют фактический сервис, а локальная статистика раздельно учитывает токены Mistral, токены и нейроны Cloudflare.
...
Мастер-пароль не сохраняется и не является паролем аккаунта Google. Если пароль забыт, расшифровать резервную копию невозможно. Настройка OAuth для собственных сборок описана в [отдельном руководстве](docs/GOOGLE-DRIVE-OAUTH.md).
