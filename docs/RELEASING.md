...
Команда собирает тестовую Chrome-версию и снимает реальный русскоязычный интерфейс без обращения к внешним AI API. Результаты:

- Chrome Web Store: `docs/store-assets/chrome/ru/` — пять скриншотов 1280×800, малый промоматериал 440×280 и marquee 1400×560;
- Firefox Add-ons: `docs/store-assets/firefox/` — семь скриншотов 1280×800;
- README: `docs/assets/lexisync-showcase-color.gif`.

Тексты карточки, примечания к выпуску и обоснования разрешений находятся в `docs/STORE-LISTING-RU.md`. Графику и тексты необходимо переносить в кабинеты магазинов вручную и сверять с `PRIVACY.md` и финальным production-манифестом.
...
3. Создать и отправить тег, совпадающий с версией, например `v5.6.4`.
