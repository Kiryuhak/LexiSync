...
let popupUI = null;
let currentTargetLang = "Английский"; // Дефолтное значение (будет меняться автоматически)
...
function handleActionClick(mode) {
    // --- УМНОЕ АВТООПРЕДЕЛЕНИЕ ЯЗЫКА ДЛЯ ПЕРЕВОДА ---
    if (mode === 'translate') {
        const text = currentSelection.text || "";
        const ruCount = (text.match(/[а-яА-ЯёЁ]/g) || []).length;
        const enCount = (text.match(/[a-zA-Z]/g) || []).length;
        
        // Если русских букв больше или равно английским - переводим на Английский. Иначе на Русский.
