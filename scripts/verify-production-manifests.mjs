...
const REQUIRED_ORIGINS = ['https://api.mistral.ai/*', 'https://api.cloudflare.com/*'];
...
for (const browser of ['chrome', 'firefox']) {
    ...
    if (!sameValues(requiredOrigins, REQUIRED_ORIGINS))
        throw new Error(`${browser}: обязательный доступ разрешён только для Mistral API и Cloudflare API`);
